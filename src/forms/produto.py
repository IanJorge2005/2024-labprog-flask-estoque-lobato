...
class ProdutoForm(FlaskForm):
    nome = StringField(label='Nome do produto', validators=[InputRequired(message="É obrigatório definir o nome do produto"),
                                                            Length(max=100, message="O produto pode ter até 100 caracteres")])

    preco = DecimalField(label='Preço', places=2, validators=[InputRequired(message="É obrigatório definir o preço"),
    ...
    ativo = BooleanField(label="Ativo?", validators=[InputRequired(message="É preciso dizer se o produto está ou não ativo")])
